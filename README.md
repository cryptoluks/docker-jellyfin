This fork uses the linuxserver focal base image and adds some dependencies needed for Intel Quick Sync hardware encoding/decoding.

Build it:

```
git clone https://github.com/cryptoluks/docker-jellyfin-focal-qsv.git
cd docker-jellyfin-focal-qsv
docker build \
  --no-cache \
  --pull \
  -t linuxserver/jellyfin-focal-qsv:latest .
```

Now you can use the image `jellyfin-focal-qsv:latest` in your `docker-compose.yml`.
